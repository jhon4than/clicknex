...
import { articleLd, breadcrumbLd, organizationLd } from '../lib/seo/jsonLd';
import { SITE_URL } from '../components/seo/siteConfig';
import { blogPosts } from './BlogIndex';
import { Clock, Calendar, ArrowLeft } from 'lucide-react';

// --- Renderizador de Markdown → HTML (headings reais, listas, blockquote, links) ---
...
// Conteúdo completo dos posts (markdown)
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-white">
        ...
        {/* Header */}
        <header className="pt-32 pb-16 bg-gradient-to-br from-blue-950 to-blue-800 text-white">
          <div className="container-custom max-w-3xl mx-auto">
            <a href="/blog" className="inline-flex items-center gap-2 text-blue-300 hover:text-white mb-6 transition-colors text-sm">
              <ArrowLeft className="w-4 h-4" /> Voltar ao blog
            </a>
            <div className="flex flex-wrap gap-2 mb-4">
              {post.tags.map((tag) => (
                <span key={tag} className="text-xs bg-blue-800 text-blue-200 px-3 py-1 rounded-full">
                  {tag}
                </span>
              ))}
            </div>
            <h1 className="text-3xl md:text-4xl font-bold mb-6 leading-tight">{post.titulo}</h1>
            <div className="flex items-center gap-6 text-blue-300 text-sm">
              ...
              <span className="flex items-center gap-1.5">
                <Clock className="w-4 h-4" /> {post.readingTime} de leitura
              </span>
            </div>
          </div>
        </header>
        ...
        <article className="py-16">
          <div className="container-custom max-w-3xl mx-auto">
            ...
            {/* CTA */}
            <div className="mt-16 p-8 bg-blue-950 rounded-2xl text-white text-center">
              <h3 className="text-2xl font-bold mb-3">Quer aplicar isso no seu negócio?</h3>
              <p className="text-blue-200 mb-6">Solicite um diagnóstico gratuito e nossa equipe monta uma estratégia personalizada para você.</p>
              <a
